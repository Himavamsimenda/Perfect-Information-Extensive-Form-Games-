...
def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    ...
    if max_player_flag:
        ...
        for action in valid_actions:
            ...
            next_history.current_player = next_history.get_current_player()
            max_val = max(max_val, alpha_beta_pruning(next_history, alpha, beta, False))
        ...
        for action in valid_actions:
            ...
            next_history.active_board_stats = next_history.check_active_boards()
            next_history.current_player = next_history.get_current_player()
            min_val = min(min_val, alpha_beta_pruning(next_history, alpha, beta, True))
...
def maxmin(history_obj, max_player_flag):
    ...
    if max_player_flag:
        ...
        for action in valid_actions:
            ...
            next_history.active_board_stats = next_history.check_active_boards()
            next_history.current_player = next_history.get_current_player()
            next_history_str = next_history.get_boards_str()
            if next_history_str not in board_positions_val_dict:
                board_positions_val_dict[next_history_str] = maxmin(next_history, False)
            max_val = max(max_val, board_positions_val_dict[next_history_str])
        return max_val
    else:
        min_val = math.inf
        valid_actions = history_obj.get_valid_actions()
        for action in valid_actions:
            ...
            next_history_str = next_history.get_boards_str()
            if next_history_str not in board_positions_val_dict:
                board_positions_val_dict[next_history_str] = maxmin(next_history, True)
            min_val = min(min_val, board_positions_val_dict[next_history_str])
        return min_val
   # return -2
    # TODO implement
