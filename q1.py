...
def backward_induction(history_obj):
    ...
    for action in valid_actions:
        # next_history = History(copy.deepcopy(history_obj.history))
    ...
    if history_obj.current_player() == 'x':
        strategy_dict_x[''.join(map(str, history_obj.history))] = {str(action): 1 if action == best_action else 0 for action in range(9)}
    else:
        strategy_dict_o[''.join(map(str, history_obj.history))] = {str(action): 1 if action == best_action else 0 for action in range(9)}
